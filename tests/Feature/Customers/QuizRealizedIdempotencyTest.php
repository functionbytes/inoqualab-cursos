...
/**
 * Regresión de QuizController::realized tras cambiar de "SELECT + create()"
 * (no atómico) a createOrFirst dentro de una transacción — el mismo patrón
 * que en CoursesController::createOrUpdateProgress (ver
 * LessonRealizedIdempotencyTest) y que dejó 413k filas duplicadas en
 * course_progress antes del índice único (inscription_id, lesson_id).
 */
class QuizRealizedIdempotencyTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_marking_the_same_lesson_twice_does_not_duplicate_progress_or_percent(): void
    {
        ...
        // type_id distinto de 6 (quiz): realized() no exige quiz aprobado.
        ...
        $marcar = fn () => $this->actingAs($customer)
            ->post(route('customers.quiz.realized'), ['lesson' => $lesson->id]);
