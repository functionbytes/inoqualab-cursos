...
class MailList extends Model
{
    ...
    public function longName($cache = false)
    {
        ...
        return $this->name.' - '.$count.' '.trans('messages.'.Tool::getPluralPrase('subscriber', $count)).'';
    }
    ...
    public function export($progressCallback = null, $segment = null)
    {
        ...
        cursorIterate($query, $orderBy = 'subscribers.id', $pageSize, function ($subscribers, $page) use ($writer, $listFields, &$processed, &$total, &$failed, &$message, $progressCallback) {
        ...
        */
        ...
        try {
            ...
            each_batch($results, config('app.import_batch_size'), false, function ($batch) use ($map, &$processed, &$failed, $total, &$overQuotaAttempt, &$message, $progressCallback, $invalidRecordCallback) {
                if ($this->customer->user->can('addMoreSubscribers', [$this, count($batch)])) {
                    $insertLimit = count($batch);
                } else {
                ...
                [$tmpTable, $emailFieldName] = $map->createTmpTableFromMapping();
                ...
                $data = array_where($data, function ($record) use (&$failed, $invalidRecordCallback, $emailFieldName) {
                    [$valid, $errors] = $this->validateCsvRecord($record, $emailFieldName);
                    if (! $valid) {
                        $failed += 1;
                ...
                $insertToSubscribersSql = strtr(
                    '
                    INSERT INTO %subscribers (uid, mail_list_id, email, status, subscription_type, tags, import_batch_id, created_at, updated_at)
    ...
    public function parseCsvFile($file, $callback)
    {
        ...
        // Read CSV files
        [$headers, $availableFields, $total, $results] = $this->readCsv($file);
    ...
    {
        [$valid, $errors] = $this->validateCsvRecord($attributes);
        if (! $valid) {
    ...
    public function sendMail($subscriber, $page, $title)
    {
        ...
        $message->setFrom([$subscriber->mailList->from_email => $subscriber->mailList->from_name]);
        $message->setTo([$subscriber->email, $subscriber->email => $subscriber->getFullName(trans('messages.to_email_name'))]);
