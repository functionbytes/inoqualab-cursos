...
namespace App\Exports\Distributors\Orders;
...
class OrdersExport implements FromQuery, Responsable, WithHeadings, WithMapping, WithStrictNullComparison
{
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            // Null-safe: enroll_expire/payment_at nulos daban "1969-12-31".
            $row->inscription_enroll_start ? date('Y-m-d', strtotime($row->inscription_enroll_start)) : '',
            $row->inscription_enroll_expire ? date('Y-m-d', strtotime($row->inscription_enroll_expire)) : '',
            $row->order_payment_at ? date('Y-m-d', strtotime($row->order_payment_at)) : '',
