...
<body>
    ...
    $signature = $signatures ? "/media/" . $signatures->id . "/" . $signatures->file_name : '';

    // D5: escalar el nombre según su longitud para que no desborde el certificado
    $fullName = trim(($user['firstname'] ?? '') . ' ' . ($user['lastname'] ?? ''));
    $nameSize = strlen($fullName) > 28 ? '22px' : (strlen($fullName) > 20 ? '26px' : '32px');

    @endphp

    <div class="contenedor" style="height: 100%; width: 100%; overflow: hidden; position: relative;">

        @if($image)
        <img src="{{  public_path() .$image }}" style="position: absolute; top: 0; left: 0; height: 100%; width: 100%; object-fit: cover;" />
        @endif
