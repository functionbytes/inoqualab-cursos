      ...
      <div class="card-body border-bottom">
        ...
        <form method="GET" action="{{ Request::url() }}" id="searchForm">
            ...
            @include('managers.includes.filter-toolbar', [
                ...
                'searchPlaceholder' => 'Buscar por orden...',
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle text-nowrap mb-0">
            ...
            @foreach ($orders as $item)
              <tr>
                <td>{{ $item->order->slack }}</td>
                <td>
                  {{-- El cliente puede haberse borrado (soft delete) después de la orden --}}
                  {{ ($item->order->user->firstname ?? 'N/D') . ' ' . ($item->order->user->lastname ?? '') }}
                </td>
                <td class="text-center">
                  <span class="badge bg-secondary-subtle text-secondary">{{ $item->order->method->title }}</span>
                </td>
                <td class="text-center">
                  <span class="badge {{ $item->order->condition->badge_class }}">{{ $item->order->condition->title }}</span>
                </td>
                <td class="text-center">
                  <span class="badge bg-secondary-subtle text-secondary">{{ $item->order->type->title }}</span>
                </td>
                <td>
                  <span class="text-muted">{{ $item->order->updated_at->format('d/m/Y') }}</span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                      <li>
                        <a class="dropdown-item" href="{{ route('accounting.orders.view', $item->order->slack) }}">Visualizar</a>
                      </li>
                    </ul>
                  </div>
                </td>
...
@push('scripts')
    <script src="{{ asset('accountings/js/views/distributors/orders/index.js') }}"></script>
@endpush
