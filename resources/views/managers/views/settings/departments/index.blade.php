...
@section('content')
  ...
  @include('managers.includes.card', ['title' => 'Departamentos'])
  ...
  <div class="widget-content searchable-container list">
    ...
    <div class="card card-body">
          ...
          <tbody>
              @foreach ($departments as $key => $department)
                <tr class="search-items">
                  <td>
                    <span class="usr-email-addr" data-email="{{ $department->title }}">{{ Str::upper( Str::lower($department->title) )  }}</span>
                  </td>
                  <td>
                      <span class="badge {{ $department->available == 1 ? 'bg-light-primary' : 'bg-light-secondary' }} rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">

                           {{ $department->available == 1 ? 'Publico' : 'Oculto' }}
                      </span>
                  </td>
                  <td>
                    <span class="usr-ph-no" data-phone="{{ date('Y-m-d', strtotime($department->updated_at)) }}">{{ date('Y-m-d', strtotime($department->updated_at)) }}</span>
                  </td>
                  <td class="text-left">
      ...
      <div class="result-body ">
        <span>Mostrar {{ $departments->firstItem() }}-{{ $departments->lastItem() }} de {{ $departments->total() }} resultados</span>
        <nav>
