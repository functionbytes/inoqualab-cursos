                    ...
                    <tbody>
                        @foreach ($documents as $key => $document)
                            <tr class="search-items">
                                ...
                                <td>
                                  <span class="badge {{ $document->available == 1 ? 'bg-light-primary' : 'bg-light-secondary' }} rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">

                                       {{ $document->available == 1 ? 'Publico' : 'Oculto' }}
                                  </span>
                                </td>
                                <td>
                                    ...
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            @if($document && $document->getfirstMedia('files'))
                                                <li>
                                                    <a class="dropdown-item d-flex align-items-center gap-3" target="_blank" href="{{ $document->getfirstMedia('files')->getfullUrl() }}">
                                                        Visualizar
                                                    </a>
                                                </li>
                                            @endif
                                            <li>
                                                <a class="dropdown-item d-flex align-items-center gap-3" href="{{ route('manager.documents.edit', $document->slack) }}">Editar</a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item d-flex align-items-center gap-3 confirm-delete" data-href="{{ route('manager.documents.destroy', $document->slack) }}">Eliminar</a>
                                            </li>
                                        </ul>
                                    </div>
                                </td>
