        ...
        $(document).ready(function() {
            ...
            $("#formDocuments").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('manager.documents.update') }}",
                        headers: {
                        ...
                        success: function(response) {
                            ...
                            if(response.success == true){
                                ...
                                setTimeout(function() {
                                        window.location = "{{ route('manager.documents') }}";
                                }, 2000);
                            }else{
            ...
            var myFiles = new Dropzone("div#files", {
                paramName: "file",
                url: "{{ route('manager.documents.files') }}",
                method: 'POST',
                addRemoveLinks: true,
                ...
                init: function() {
                    ...
                    $.getJSON("{{ route('manager.documents.files.get', ':item') }}".replace(':item', item), function(data) {
                        if (data.length > 0) {
                            $("#status").val('true');
