...
@section('content')
  ...
  @include('managers.includes.card', ['title' => 'Aliados'])
    ...
    <div class="card card-body">
      <div class="table-responsive">
        <table class="table search-table align-middle text-nowrap">
          ...
          <tbody>
              @foreach ($trusteds as $key => $trusted)
                <tr class="search-items">
                  <td>
                    <span class="usr-email-addr" data-email="{{ $trusted->title   }}">{{ $trusted->title  }}</span>
                  </td>
                  <td>
                     <span class="badge {{ $trusted->available == 1 ? 'bg-light-primary' : 'bg-light-secondary' }} rounded-3 py-2 text-primary fw-semibold fs-2 d-inline-flex align-items-center gap-1">
                          {{ $trusted->available == 1 ? 'Publico' : 'Oculto' }}
                    </span>
                  </td>
                  <td>
                    <span class="usr-ph-no" data-phone="{{ date('Y-m-d', strtotime($trusted->updated_at)) }}">{{ date('Y-m-d', strtotime($trusted->updated_at)) }}</span>
                  </td>
                  <td class="text-left">
      ...
      <div class="result-body ">
        <span>Mostrar {{ $trusteds->firstItem() }}-{{ $trusteds->lastItem() }} de {{ $trusteds->total() }} resultados</span>
        <nav>
