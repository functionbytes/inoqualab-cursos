...
@section('page_header')
    @include('supports.includes.card', ['title' => 'Editar empleado'])
@endsection
@section('content')
  ...
  <div class="row g-4 align-items-start">
    ...
    <div class="col-lg-8">
      ...
      <form id="formUsers" enctype="multipart/form-data" role="form"
            data-update-url="{{ route('support.enterprises.staffs.update') }}"
            data-redirect-url="{{ route('support.enterprises.staffs', $enterprise->slack) }}">

        {{ csrf_field() }}

        <input type="hidden" id="id" name="id" value="{{ $user->id }}">
        <input type="hidden" id="slack" name="slack" value="{{ $user->slack }}">
        {{-- Sin este fallback, $distributor->slack lanza un 500 si la empresa no tiene distribuidor asociado. --}}
        ...
        <div class="card">
          ...
          <div class="card-body">
            <h6 class="fw-bold text-dark mb-1">Editar empleado</h6>
...
@push('scripts')
    <script src="{{ asset('supports/js/enterprises/staffs/edit.js') }}"></script>
@endpush
