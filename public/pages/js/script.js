...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        if ($('.main-header .navigation li.dropdown ul').length) {
            $('.main-header .navigation li.dropdown').append('<div class="dropdown-btn"><span class="fa-duotone fa-arrow-down"></span></div>');
        ...
        if ($('.coach-slider').length) {
            // slidesToShow no debe superar el nº de slides: si lo hace con infinite:true,
            // slick falla en initADA ("reading 'add'" sobre un slide clonado inexistente).
            var coachSlides = $('.coach-slider').children().length;
            $('.coach-slider').slick({
                slidesToShow: Math.min(3, coachSlides),
                slidesToScroll: 1,
                infinite: coachSlides > 3,
                speed: 400,
                autoplay: true,
                arrows: false,
                dots: true,
    ...
    $(window).on('load', function () {
        ...
        // .coach-slider ya se inicializa en document.ready (sección "12. Coach Slider").
        // Se elimina la doble inicialización que rompía slick (initADA) al re-inicializar.
